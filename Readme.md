...
> **What makes this unusual:** GFX1013 silicon informally called "RDNA 1.5" by the community. ROCm's userspace libraries don't ship GFX1013 support. OpenCL/rusticl was not functional in this configuration. On this Fedora 43 / Mesa 25.3.4 stack, Vulkan was the only GPU compute path found to be usable at the time — and even that required working around two kernel memory bottlenecks (GTT cap + TTM pages_limit) before 14B models would run. ROCm compute has since been made to work with a patched kernel and specific configuration, documented in [bc250-rocm](https://github.com/akandr/bc250-rocm); Vulkan remains the simpler and faster inference path. The GFX1013 die physically has 40 compute units, but the factory firmware exposes only 24; a community kernel patch re-enables the rest.
>
...
| **ROCm / HIP** | ⚠️ | Not usable as shipped; works on kernel 7.1.5 with patches and specific configuration. Details in [bc250-rocm](https://github.com/akandr/bc250-rocm) |
| **OpenCL (rusticl)** | ⚠️ | Not usable in this configuration |

**Why stock ROCm fails:** GFX1013 is listed in LLVM as supporting `rocm-amdhsa`, but AMD's ROCm userspace (rocBLAS/Tensile) doesn't ship GFX1013 solution libraries (the companion repo later shows they can be built natively for GFX1013). On this Fedora 43 / Mesa 25.3.4 deployment, Vulkan was the only GPU compute path found to work.

A dedicated HIP/ROCm investigation later traced this further and, as of August 2026, reached a working state: on kernel 7.1.5 with a corrected TLB flush, hardware scheduling, the 40-CU unlock, and a flush-on-unmap workaround, ROCm compute runs verified-correct GEMM, PyTorch matmul, FP64, and text-verified llama.cpp inference on this board. The failures documented earlier (Mesa's `GFX1013 is known to have broken compute queue`, intermittent hangs and wrong results) were real on the older stack and are explained there; Vulkan keeps a roughly 10x prompt-processing advantage and remains the practical default for inference here. The full write-up, reproducers, configuration, and the HIP-vs-Vulkan comparison are in the companion repository: [github.com/akandr/bc250-rocm](https://github.com/akandr/bc250-rocm).
